...
function saveSchedule(job) {
  ...
  try { fs.writeFileSync(SCHEDULES_FILE, JSON.stringify(schedules, null, 2), 'utf-8'); } catch (_) {}
  logAuditEvent({
    role: job.role || 'admin',
...
function logAuditEvent(event) {
  try {
    ...
    logs.unshift(entry);
    try { fs.writeFileSync(AUDIT_FILE, JSON.stringify(logs.slice(0, 100), null, 2), 'utf-8'); } catch (_) {}
    return entry;
  } catch (err) {
...
function saveCustomReport(report, role = 'cxo') {
  ...
  try { fs.writeFileSync(path.join(DATA_DIR, 'customReports.json'), JSON.stringify(reports, null, 2), 'utf-8'); } catch (_) {}
  logAuditEvent({
    role,
